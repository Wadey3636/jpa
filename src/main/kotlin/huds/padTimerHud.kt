...
class padTimerHud : Hud(stormActivated) {
    ...
    override fun draw(matrices: UMatrixStack?, x: Float, y: Float, scale: Float, example: Boolean) {
        ...
        if (padticks % 2 != 1f) {
            TextRenderer.drawScaledString((padticks / 20).toString() + "0", x, y, padcolor.getRGB(), TextRenderer.TextType.toType(0), scale)
        } else {
        TextRenderer.drawScaledString((padticks / 20).toString(), x, y, padcolor.getRGB(), TextRenderer.TextType.toType(0), scale)
        }
